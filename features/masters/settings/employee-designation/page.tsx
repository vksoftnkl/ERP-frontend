...
/**
 * The Grid Master row this list reads — "MAIN LIST - EMP DESIGNATIONS". `CrudMasterPage`
 * resolves it to a grid id at runtime (see lib/configured-grids), and uses it
 * for both the rows and the configured columns.
 */
const LIST_GRID_KEY = "employeeDesignationList" satisfies ConfiguredGridKey;
const API_ENDPOINTS = {
...
const WIDGET_SECTION_MENU_ID = 71;
...
// stored section_platform exactly — the designation fields live on the "Web" section.
const WIDGET_SECTION_PLATFORM = "Web";
// Bridge each hardcoded form field `name` to the backend `fieldName` it is
// configured under on menu 71's "Web" section (matched case-insensitively).
const WIDGET_FIELD_NAME_BY_FORM_FIELD: Record<string, string> = {
  masterName: "designation_name",
  edCode: "designation_code",
  masterDescription: "designation_remarks",
  edIsDefault: "designation_default",
  edIsActive: "designation_status",
};
...
export default function EmployeeDesignationMasterPage() {
  ...
  // can see soft-deleted designations. Lives beside the list search input.
  ...
  // grid 24's stored SQL; keys with no matching token are ignored. `wantdelete` is
  ...
  const designationFormFields = useMemo(
    () =>
      applyWidgetFieldConfig(
        DESIGNATION_FORM_FIELDS,
        widgetFieldConfig,
  ...
  // "Web" platform to match the configured designation section.
  ...
  return (
    ...
    <CrudMasterPage
      ...
      customFields={designationFormFields}
