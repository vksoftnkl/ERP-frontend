...
function extractRows(payload: unknown, arrayKeys: readonly string[]): RawRecord[] {
  ...
  if (payload && typeof payload === "object") {
    for (const key of arrayKeys) {
      ...
      // API returns { data: { items: [...] } } — look one level deeper
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (event: KeyboardEvent) => {
      ...
      if (event.key === "F5") {
        ...
        if (!loading && companyId && branchId) handleSubmit();
      }
    };
    ...
    <ModalPortal>
      <div className={dynamicModalStyles.overlay}>
        ...
        <section
          ...
          style={{ width: "min(500px, calc(calc(100vw/var(--erp-ui-scale)) - 2rem))", maxHeight: "min(calc(92vh/var(--erp-ui-scale)), 620px)" }}
          role="dialog"
          aria-modal="true"
