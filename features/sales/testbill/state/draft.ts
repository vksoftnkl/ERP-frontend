/**
 * Sale Bill — the draft reducer (§5.1): ONE draft, ONE reducer, driven by
 * `useReducer` inside `useBillDraft`. It is built with RTK's `createSlice`
 * for the Immer ergonomics and the typed action creators, but it is NOT
 * registered in the store: the bill's state lives with the screen and dies
 * with it, so a second bill screen never shares a draft with the first.
 *
 * Every transition delegates to the pure functions in `state/factories.ts`
 * and `domain/*`; the trailing-blank-row invariant is applied last, and the
 * wrapper stamps `isDirty` / flips `pricing` to `live` on the first real edit.
 *
 * One rule is this screen's own and is worth finding here rather than by
 * reading every case: **changing the customer is guarded, not reactive**
 * (§7.1). A bill that already carries tendered money or adjustments loses
 * both when the party changes, so `customerApplied` refuses to run while they
 * stand and the screen must clear them first — through
 * `customerBoundStateCleared`, which is what the confirmation dialog
 * dispatches. Expressed as a guard rather than as an effect that reacts to a
 * changed id, because the difference is whether the operator is asked before
 * their settlement disappears or after (§5.3: derived values are computed in
 * the reducer, never in an effect).
 */
...
import { applyBillLifecycle, applyBillSaveResponse } from "@/features/sales/testbill/domain/load-map";
import { overridesAgainst } from "@/features/sales/testbill/domain/notes";
import { applyCarryProposals, carryAfterAmountEdit, setCarryBasis } from "@/features/sales/testbill/domain/carry";
import {
  adjustLinePrices,
...
import { clampDueDays, partyTerm } from "@/features/sales/testbill/domain/party";
import type {
  AdjustableCredit,
  BillAdjustmentRow,
  BillChargeRow,
  ...
  reducers: {
    ...
    itemPriceApplied(
      ...
      action: PayloadAction<{
        ...
        /**
         * A challan / order line keeps its SOURCE rate (§28 Q5): the lookup
         * fills everything else — stock, godown, tax, cost, MRP — and leaves
         * the rate the document promised. `actualPrice` still takes the list
         * price, so the savings figure stays honest.
         */
        keepRate?: boolean;
      }>,
    ) {
      const { key, lookup, unitName, unitId, autoPopQty, pending, keepRate } = action.payload;
      const index = state.lines.findIndex((row) => row.key === key);
      if (index < 0) {
        return;
      }
      const sourceRate = state.lines[index].rate;
      let line = applyBillItemPrice(state.lines[index] as SaleBillDraftLine, lookup, {
        unitName,
        unitId,
      });
      if (keepRate && sourceRate > 0) {
        line = { ...line, rate: sourceRate };
      }
    ...
    chargeFieldSet(
      ...
      if (field === "amount") {
        row.rate = 0;
        // An Amount edit makes a carried row MANUAL (§12.1, §12.4).
        const index = state.charges.findIndex((candidate) => candidate.key === key);
        state.charges[index] = carryAfterAmountEdit(row as BillChargeRow);
      }
      if (field === "rate") {
        row.amount = 0;
      }
    },
    /** `/validate`'s charge carry proposals, applied to the carried rows (§12.4). */
    chargeCarryProposalsApplied(state, action: PayloadAction<ChargeCarryProposal[]>) {
      const result = applyCarryProposals(state.charges as BillChargeRow[], action.payload);
      if (result.changed) {
        state.charges = result.rows;
      }
    },
    /** The per-row basis menu: PRORATA · FULL · MANUAL · NONE (§12.4). */
    chargeCarryBasisSet(state, action: PayloadAction<{ key: string; basis: ChargeCarryBasis }>) {
      const index = state.charges.findIndex((row) => row.key === action.payload.key);
      if (index < 0) {
        return;
      }
      state.charges[index] = setCarryBasis(state.charges[index] as BillChargeRow, action.payload.basis);
    },
    /**
     * Lines appended from the Open Sources dialog (§13.4) — the whole draft
     * the pure append produced, because it moves the trail and the chips as
     * well as the lines. Appends, never replaces.
     */
    linesAppended(_state, action: PayloadAction<SaleBillDraft>) {
      return action.payload;
    },
    /** The identity strip's chips (§13.6), kept client-side until `/get` replaces them. */
    sourcesSet(state, action: PayloadAction<BillSourceSummary[]>) {
...
const NON_EDIT_ACTIONS = new Set<string>([
  ...
  amendBegun.type,
  amendAbandoned.type,
  // A carry proposal is the server's answer arriving, not an edit; the chips
  // likewise are bookkeeping about documents already imported.
  chargeCarryProposalsApplied.type,
  sourcesSet.type,
...
export type BillAction = { type: string; payload?: unknown };

/** The reducer `useBillDraft` hands to `useReducer`: a total function over a draft. */
export function billReducer(state: SaleBillDraft, action: BillAction): SaleBillDraft {
  return saleBillReducer(state, action);
}
