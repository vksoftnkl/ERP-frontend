...
import { DEFAULT_BILL_STATUS, DEFAULT_BILL_TYPE } from "@/features/sales/testbill/constants";
import {
...
import type { SaleBillDraft, SaleBillDraftLine } from "@/features/sales/testbill/types";
import {
  draftReplaced,
  lineDuplicated,
  modeSet,
...
describe("resetBillDraft — Clear (F7) in every state", () => {
  it("keeps the crew but never the agent when the setting says so, else drops everything", async () => {
    const { resetBillDraft } = await import("@/features/sales/testbill/state/factories");
    const base = createBillDraft(CONTEXT);
...
describe("Disc % All and ± Price (§6.2)", () => {
  it("Disc % All clears the per-qty and amount tiers on every non-free line", async () => {
    const { applyDiscountToAllLines } = await import("@/features/sales/testbill/state/factories");
  ...
  it("± Price scales the rate on non-free lines; 0 is a no-op", async () => {
    const { adjustLinePrices } = await import("@/features/sales/testbill/state/factories");
...
describe("applyPartyContext — one writer per field (§7.1)", () => {
  it("writes the standing, `creditAllowed` and the loyalty member, and only falls back on the rest", async () => {
    const { applyPartyContext } = await import("@/features/sales/testbill/state/factories");
    const { parsePartyContext } = await import("@/features/sales/testbill/domain/party");
    const base = createBillDraft(CONTEXT);
    const draft: SaleBillDraft = {
      ...base,
