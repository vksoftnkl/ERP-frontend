"use client";
/**
 * Sale Bill — the screen's whole conversation with the server, and the one
 * place `recalcDocument` is called. The draft is a local `useReducer` over
 * `state/draft.ts` (§5.1); every thunk below dispatches to it.
 *
...
} from "@/features/sales/testbill/api/gst";
import { appendOpenSourceLines, type OpenSourcePick } from "@/features/sales/testbill/domain/import";
import { transportBandDto, transportBandIsEmpty } from "@/features/sales/testbill/domain/shipping";
import { buildRetenderBody, type VoidPick } from "@/features/sales/testbill/retender/retender-payload";
import { reconcileNoticeText, reconcileRestoredAdjustments } from "@/features/sales/testbill/engines/adjust";
...
import { creditAlertMessage, termForcedToParty } from "@/features/sales/testbill/domain/party";
import { clampAdjustmentsToBill, mergeHeldCredits } from "@/features/sales/testbill/engines/adjust";
import { releaseAdjustments, rollupsOf } from "@/features/sales/testbill/engines/settle";
import type { SettleResult } from "@/features/sales/testbill/components/settle-dialog";
...
import {
  ...
  reconcileFreeLines,
  type PromotionNote,
} from "@/features/sales/testbill/engines/promotions/evaluate";
import type { PromotionContext } from "@/features/sales/testbill/engines/promotions/match";
import {
  adjustmentsForWire,
  billKeyOf,
  buildAmendBody,
...
} from "@/features/sales/testbill/domain/validate";
import {
  creditsRaisedBy,
  importOrder,
  importQuotation,
...
import {
  ...
  writeAutosave,
  type BillAutosave,
} from "@/features/sales/testbill/state/hold";
import { confirm } from "@/lib/confirm";
import { amendBlockedReason, cancelBlockedReason } from "@/features/sales/testbill/state/verbs";
import {
  errorCodeOf,
  errorMessageOf,
  errorStatusOf,
...
import { SALESMAN_DROPDOWN_KEY } from "@/features/sales/quotation/quotation.constants";
/**
...
export type SaleBillDraftApi = {
  draft: SaleBillDraft;
  /** The draft's own reducer dispatch (§5.1) — one `useReducer`, not a store. */
  ...
  copyAsNew: () => void;
  beginEdit: () => void;
  // ----- open sources (§13.3, §13.4) -----
  /** The dialog's state: what was asked for, what came back. */
  openSources: { kind: "DC" | "ORDER"; docs: OpenSourceDoc[]; loading: boolean; error: string | null } | null;
  /** Ctrl+F5 (DC) / Ctrl+F4 on a bill with lines (ORDER): gate, then read the party's open documents. */
  requestOpenSources: (kind: "DC" | "ORDER") => Promise<void>;
  closeOpenSources: () => void;
  /** The dialog's Take: append the picked lines and price the new ones (source rate kept). */
  applyOpenSources: (picks: OpenSourcePick[]) => Promise<void>;
  // ----- charge carry (§12.4) -----
  setCarryBasis: (chargeKey: string, basis: ChargeCarryBasis) => void;
  // ----- shipping (§20) -----
  branchAddress: BranchAddress | null;
  loadBranchAddress: () => Promise<void>;
  /** `document` mode: the band, the terms and the vehicle onto the draft. */
  applyShipping: (result: { band: TransportBand; terms: SaleBillTerms; vehicleNo: string; vehicleId: string | null }) => void;
  /** `ownVerb` mode: PUT `/bills/transport`. */
  saveTransport: (band: TransportBand) => Promise<boolean>;
  /** Bumped when a validate note marks the shipping dialog required (§16.5) — the screen opens it. */
  transportRequiredTick: number;
  requireTransport: (message: string) => void;
  // ----- GST (§21) -----
  gstBusy: boolean;
  einvoice: { label: string; enabled: boolean; tooltip: string | null };
  ewaybill: { label: string; enabled: boolean; isPartB: boolean };
  generateEinvoice: () => Promise<void>;
  generateEwaybill: () => Promise<boolean>;
  updateEwaybillVehicle: (vehicleNo: string, remark: string | null) => Promise<void>;
  // ----- quick add (§7.7) -----
  quickAddBusy: boolean;
  /** `POST /customers/create`; resolves the new id, or null when refused. */
  quickAddCustomer: (dto: QuickAddCustomerDto) => Promise<string | null>;
  // ----- re-tender (§22) -----
  retender: { key: BillKey; refno: string | null; context: TenderContext | null; loading: boolean } | null;
  openRetender: (key: BillKey, refno: string | null) => Promise<void>;
  closeRetender: () => void;
  submitRetender: (voids: VoidPick[], replacements: BillTenderRow[], remark: string) => Promise<boolean>;
  // ----- printing (§26) -----
  /** The bill this screen last wrote, for F11's reprint. */
  lastBillRef: SavedBillRef | null;
};
/**
...
export function useSaleBillDraft(): SaleBillDraftApi {
  ...
  // ONE draft, ONE reducer (§5.1). The state lives with the screen and dies
  // with it; the Redux store is only reached for the RTK Query endpoints.
      ...
      try {
        ...
        dispatch(
          itemPriceApplied({
            ...
            pending,
            keepRate: options.keepRate === true,
          }),
        );
  ...
  /**
   * §16.5 — `SALES_EWAY_TRANSPORT_MISSING` / `GST_EWB_TRANSPORT_MISSING`
   * mark the shipping dialog required and OPEN it (a WARN that cannot be
   * overridden). The server reads the band from the database, so an unsaved
   * band warns falsely until the draft is saved (§27 E-WAY-DB).
   */
  const [transportRequiredTick, setTransportRequiredTick] = useState(0);
  const requireTransport = useCallback(
    (message: string) => {
      dispatch(transportRequiredSet(message));
      setTransportRequiredTick((tick) => tick + 1);
    },
    [dispatch],
  );
  const noteTransportRequirement = useCallback(
    (notes: ReturnType<typeof parseValidationNotes>) => {
      const missing = transportMissingNote(notes);
      if (missing) {
        requireTransport(missing.message);
      } else if (draftRef.current.transportRequired) {
        dispatch(transportRequiredSet(null));
      }
    },
    [dispatch, requireTransport],
  );
  ...
  const runServerValidation = useCallback(
    async (payload: ReturnType<typeof buildSavePayload>, options: { popupOnRefusal: boolean }): Promise<ServerValidation> => {
      try {
        ...
        // The charge carry proposals (§12.4) ride on a successful validate.
        const proposals = parseCarryProposals({ data: result });
        if (proposals.length > 0) {
          dispatch(chargeCarryProposalsApplied(proposals));
        }
        noteTransportRequirement(notes);
    ...
    [dispatch, noteTransportRequirement, validateBill],
  );
  ...
  /** The bill this screen last wrote — F11 reprints it (§26). */
  const [lastBillRef, setLastBillRef] = useState<SavedBillRef | null>(null);
  ...
  // -------------------------------------------------------------------------
  /**
   * Select by id, resolve the label (§6.4 gap 2). `/bills/get` returns the
   * crew as ids only; one read of the employee dropdown names them. Best
   * effort: a name that does not resolve leaves the id's field blank, which
   * the operator can re-pick.
   */
  const resolveCrewNames = useCallback(
    async (loaded: SaleBillDraft) => {
      const people = loaded.header.people;
      const wanted: Array<[keyof typeof people, keyof typeof people, string | null]> = [
        ["salesmanId", "salesmanName", people.salesmanId],
        ["agentId", "agentName", people.agentId],
        ["driverId", "driverName", people.driverId],
        ["loadmanId", "loadmanName", people.loadmanId],
        ["packedId", "packedName", people.packedId],
        ["supervisorId", "supervisorName", people.supervisorId],
      ];
      const ids = wanted.filter(([, , id]) => Boolean(id));
      if (ids.length === 0) {
        return;
      }
      try {
        const page = await runDropdown({
          dropdownId: getDropdownId(SALESMAN_DROPDOWN_KEY),
          search: "",
          limit: 100,
          params: { iemp_branch_id: loaded.branchId },
        }).unwrap();
        const names = new Map((page.items ?? []).map((row) => [String(row.emp_id ?? ""), String(row.emp_name ?? "")]));
        if (draftRef.current.docId !== loaded.docId) {
          return;
        }
        for (const [, nameField, id] of ids) {
          const name = id ? names.get(id) : undefined;
          if (name) {
            dispatch(peopleFieldSet({ field: nameField, value: name }));
          }
        }
      } catch {
        // The ids still travel on the payload; only the labels are missing.
      }
    },
    [dispatch, runDropdown],
  );
  ...
  const loadDocument = useCallback(
    async (key: SaleBillDocKey): Promise<SaleBillDraft | null> => {
      ...
      try {
        ...
        const payload = await storeDispatch(
        ...
        // `/get` carries no staff display names (§27 NAMES): the crew's
        // labels are resolved by id from the employee dropdown (§6.4 gap 2).
        void resolveCrewNames(loaded);
        return loaded;
      } catch (error) {
        toast.error(errorMessage(error));
        return null;
    ...
    [companyStateCode, dispatch, readPartyContext, resolveCrewNames, storeDispatch],
  );
  ...
  // -------------------------------------------------------------------------
  /**
   * A restored bill — a hold pulled back or a crash recovery — reconciles
   * its set-offs against TODAY's open credits (§14.5): rows no longer open
   * are dropped, rows with less left are reduced, and ONE notice lists them
   * all. Then party-context is re-read (§23.1): Qt forgets to, and its
   * restore can race the walk-in seed.
   */
  const reconcileRestored = useCallback(
    async (restored: SaleBillDraft) => {
      if (restored.adjustments.length > 0 && restored.customer.custId && restored.companyId) {
        try {
          const credits = await fetchOpenCredits({
            partyId: restored.customer.custId,
            companyId: restored.companyId,
          }).unwrap();
          dispatch(openCreditsSet(credits));
          const result = reconcileRestoredAdjustments(restored.adjustments, credits);
          if (result.notices.length > 0) {
            dispatch(adjustmentsApplied({ rows: result.rows, from: "bill" }));
            const text = reconcileNoticeText(result.notices);
            if (text) {
              toast.warn(text);
            }
          }
        } catch {
          // The panel says "Open credits aren't available just now" when opened.
        }
      }
      await readPartyContext({
        custId: restored.customer.custId,
        companyId: restored.companyId,
        branchId: restored.branchId,
        accYear: restored.accYear,
        billDate: restored.header.billDate,
      });
    },
    [dispatch, fetchOpenCredits, readPartyContext],
  );
  ...
  const resumeHold = useCallback(
    async (txhId: string): Promise<boolean> => {
      ...
      try {
        ...
        const restored = draftFromBillHold(hold, state);
        // Restore, THEN mark the hold active (§23.1): the restore resets the
        // screen, and a reset releases any active lease.
        dispatch(draftReplaced(restored));
        await reconcileRestored(restored);
        return true;
      } catch (error) {
    ...
    [actor.deviceMasterId, dispatch, fetchHold, reconcileRestored, resumeHoldLock],
  );
  ...
  // -------------------------------------------------------------------------
  const [fetchOpenSources] = useLazyGetOpenSourcesQuery();
  const [openSources, setOpenSources] = useState<SaleBillDraftApi["openSources"]>(null);
  const requestOpenSources = useCallback(
    async (kind: "DC" | "ORDER") => {
      const current = draftRef.current;
      if (current.mode !== "entry") {
        toast.warn("This bill is read-only.");
        return;
      }
      if (!current.customer.custId) {
        toast.warn("Pick the customer first — only their open documents are offered.");
        document.getElementById("sale-bill-customer")?.focus();
        return;
      }
      // A second order needs `sales.multi_order_bill` (§13.3).
      const hasOrderLines = current.lines.some((line) => line.itemId && line.srcDocType === "SALES_ORDER");
      if (kind === "ORDER" && hasOrderLines && !settingsRef.current.multiOrderBill) {
        toast.warn("This company bills one order per bill. Clear the bill first, or turn on sales.multi_order_bill.");
        return;
      }
      setOpenSources({ kind, docs: [], loading: true, error: null });
      try {
        const docs = await fetchOpenSources({
          companyId: current.companyId,
          branchId: current.branchId,
        ...
        setOpenSources({ kind, docs, loading: false, error: null });
      } catch (error) {
        // An error is shown as status text in the dialog, not a modal.
        setOpenSources({ kind, docs: [], loading: false, error: errorMessageOf(error) });
      }
    },
    [fetchOpenSources],
  );
  const closeOpenSources = useCallback(() => setOpenSources(null), []);
  const applyOpenSources = useCallback(
    async (picks: OpenSourcePick[]) => {
      const kind = openSources?.kind;
      setOpenSources(null);
      if (!kind || picks.length === 0) {
        return;
      }
      const outcome = appendOpenSourceLines(draftRef.current, kind, picks);
      dispatch(linesAppended(outcome.draft));
      const parts: string[] = [];
      if (outcome.newLineKeys.length > 0) {
        parts.push(`${outcome.newLineKeys.length} line${outcome.newLineKeys.length === 1 ? "" : "s"} taken`);
      }
      if (outcome.toppedUp > 0) {
        parts.push(`${outcome.toppedUp} topped up`);
      }
      if (outcome.clamped > 0) {
        parts.push(`${outcome.clamped} held at the order quantity`);
      }
      if (parts.length > 0) {
        toast.info(parts.join(" · ") + ".");
      }
      // The lookup fills the rest of every new line and KEEPS the source
      // rate (§28 Q5). Sequential, so the grid does not storm the server.
      for (const key of outcome.newLineKeys) {
        const line = outcome.draft.lines.find((row) => row.key === key);
        if (line?.itemId) {
          await pickItem(key, line.itemId, line.itemUnitId || undefined, null, { keepRate: true });
        }
      }
    },
    [dispatch, openSources?.kind, pickItem],
  );

  // -------------------------------------------------------------------------
  // Charge carry (§12.4)
  // -------------------------------------------------------------------------
  const setCarryBasis = useCallback(
    (chargeKey: string, basis: ChargeCarryBasis) => {
      dispatch(chargeCarryBasisSet({ key: chargeKey, basis }));
    },
    [dispatch],
  );
  ...
  // Shipping (§20)
  // -------------------------------------------------------------------------
  const [fetchBranchAddress] = useLazyGetBranchAddressQuery();
  const [saveBillTransport] = useSaveBillTransportMutation();
  const [branchAddress, setBranchAddress] = useState<BranchAddress | null>(null);
  const loadBranchAddress = useCallback(async () => {
    const branchId = draftRef.current.branchId;
    if (!branchId || branchAddress?.brId === branchId) {
      return;
    }
    try {
      setBranchAddress(await fetchBranchAddress(branchId).unwrap());
    } catch {
      // The dispatch block stays blank; the godown is still pickable.
    }
  }, [branchAddress?.brId, fetchBranchAddress]);
  const applyShipping = useCallback(
    (result: { band: TransportBand; terms: SaleBillTerms; vehicleNo: string; vehicleId: string | null }) => {
      const current = draftRef.current;
      dispatch(transportSet(result.band));
      if (current.transportRequired && !transportBandIsEmpty(result.band)) {
        dispatch(transportRequiredSet(null));
      }
      for (const field of ["paymentTerms", "deliveryTerms", "termsConditions"] as const) {
        if (current.terms[field] !== result.terms[field]) {
          dispatch(termsFieldSet({ field, value: result.terms[field] }));
        }
      }
      if (current.header.people.vehicleNo !== result.vehicleNo) {
        dispatch(peopleFieldSet({ field: "vehicleNo", value: result.vehicleNo }));
      }
      if (current.header.people.vehicleId !== result.vehicleId) {
        dispatch(peopleFieldSet({ field: "vehicleId", value: result.vehicleId }));
      }
    },
    [dispatch],
  );
  const saveTransport = useCallback(
    async (band: TransportBand): Promise<boolean> => {
      const current = draftRef.current;
      const key = billKeyOf(current);
      if (!key) {
        toast.warn("Save the bill first — the transport verb is for a posted bill.");
        return false;
      }
      setBusy("saving");
      try {
        await saveBillTransport({ ...key, transport: transportBandDto(band) }).unwrap();
        dispatch(transportSet(band));
        dispatch(transportRequiredSet(null));
        toast.success(`Transport details saved on bill ${current.billRefno || ""}.`.replace("  ", " "));
        return true;
      } catch (error) {
        if (errorCodeOf(error) === "GST_DECLARED_LOCKED") {
          // An IRN or EWB landed while the popup was open: reload the bill.
          toast.error("An e-invoice or e-way bill was generated while the popup was open — the bill is reloaded.");
          await loadDocumentRef.current(key);
          return false;
        }
        const notes = notesFromError(error);
    ...
    [dispatch, saveBillTransport],
  );

  // -------------------------------------------------------------------------
  // GST (§21)
  // -------------------------------------------------------------------------
  const [generateEinvoiceMutation, { isLoading: einvoiceBusy }] = useGenerateEinvoiceMutation();
  const [generateEwaybillMutation, { isLoading: ewaybillBusy }] = useGenerateEwaybillMutation();
  const [updateEwaybillVehicleMutation, { isLoading: partBBusy }] = useUpdateEwaybillVehicleMutation();
  const gstBusy = einvoiceBusy || ewaybillBusy || partBBusy;
  const posted = draft.status === "POSTED";
  const irnStatus = draft.posting?.irn.status ?? "NA";
  const ewbStatus = draft.posting?.ewb.status ?? "NA";
  const einvoice = useMemo(
    () => ({
      label: irnStatus === "FAILED" || irnStatus === "REJECTED" ? "Retry e-Inv" : irnStatus === "PENDING" ? "Fetch e-Inv" : "e-Inv",
      enabled: posted && irnStatus !== "GENERATED" && !gstBusy,
      tooltip:
        irnStatus === "GENERATED"
          ? "IRN already generated. Cancel it from the bill's Cancel (inside the window) or the register (Shift+F2)."
          : null,
    }),
    [gstBusy, irnStatus, posted],
  );
  const ewaybill = useMemo(
    () => ({
      label: ewbStatus === "GENERATED" ? "Part-B" : ewbStatus === "FAILED" || ewbStatus === "REJECTED" ? "Retry e-Way" : "e-Way",
      enabled: posted && !gstBusy,
      isPartB: ewbStatus === "GENERATED",
    }),
    [ewbStatus, gstBusy, posted],
  );
  /** An error goes to the strip FIRST; the generic box only if the strip stayed empty (§21). */
  const gstFailure = useCallback(
    (error: unknown) => {
      const notes = notesFromError(error);
      if (notes.length > 0) {
        dispatch(notesSet({ notes }));
    ...
    [dispatch, noteTransportRequirement],
  );
  const generateEinvoice = useCallback(async () => {
    const current = draftRef.current;
    const key = billKeyOf(current);
    if (!key || current.status !== "POSTED") {
      toast.warn("Only a posted bill gets an e-invoice.");
      return;
    }
    if (transportBandIsEmpty(current.transport) && !current.header.people.vehicleNo.trim()) {
      const ok = await confirm({
        title: "No transport details",
        message: "No transport details on this bill. Generate the IRN without an e-way bill?",
    ...
    try {
      const result = (await generateEinvoiceMutation({ ...key, sync: true }).unwrap()) as Record<string, unknown>;
      const irn = typeof result.irn === "string" ? result.irn : typeof result.number === "string" ? result.number : null;
      const status = typeof result.status === "string" ? result.status : null;
      if (irn) {
        toast.success(`IRN generated: ${irn}`);
      } else {
        toast.info(`e-Invoice ${status ?? "answered"}: ${typeof result.message === "string" ? result.message : "see the register"}`);
      }
      await loadDocumentRef.current(key);
    } catch (error) {
      gstFailure(error);
    }
  }, [generateEinvoiceMutation, gstFailure]);
  const generateEwaybill = useCallback(async (): Promise<boolean> => {
    const current = draftRef.current;
    const key = billKeyOf(current);
    if (!key || current.status !== "POSTED") {
      toast.warn("Only a posted bill gets an e-way bill.");
      return false;
    }
    if (transportBandIsEmpty(current.transport)) {
      // Mark the shipping dialog required and open it (§21).
      requireTransport("An e-way bill needs the transport details — fill them and save first.");
      return false;
    }
    try {
      const result = (await generateEwaybillMutation({ ...key, sync: true }).unwrap()) as Record<string, unknown>;
      const number = typeof result.ewbNo === "string" ? result.ewbNo : typeof result.number === "string" ? result.number : "";
      const validUpto = typeof result.validUpto === "string" ? result.validUpto : "";
      toast.success(`E-way bill ${number} generated${validUpto ? `, valid to ${validUpto.slice(0, 10)}` : ""}.`);
      await loadDocumentRef.current(key);
      return true;
    } catch (error) {
      gstFailure(error);
  ...
  }, [generateEwaybillMutation, gstFailure, requireTransport]);
  const updateEwaybillVehicle = useCallback(
    async (vehicleNo: string, remark: string | null) => {
      const current = draftRef.current;
      const key = billKeyOf(current);
      const registerId = current.posting?.registerId;
      if (!key || !registerId) {
        toast.warn("This bill has no GST register row to update.");
        return;
      }
      try {
        await updateEwaybillVehicleMutation({
          gdrId: registerId,
          vehicleNo: vehicleNo.trim().toUpperCase(),
          mode: current.transport.mode || null,
          reasonCode: 1,
          remark: remark?.trim() || null,
          fromPlace: current.transport.from.place ?? null,
          fromState: current.companyStateCode || null,
        }).unwrap();
        toast.success(`Vehicle updated to ${vehicleNo.trim().toUpperCase()} on e-way bill ${current.posting?.ewb.number ?? ""}.`);
        await loadDocumentRef.current(key);
      } catch (error) {
        gstFailure(error);
      }
    },
    [gstFailure, updateEwaybillVehicleMutation],
  );

  // -------------------------------------------------------------------------
  // Quick add (§7.7)
  // -------------------------------------------------------------------------
  const [quickAddMutation, { isLoading: quickAddBusy }] = useQuickAddCustomerMutation();
  const quickAddCustomer = useCallback(
    async (dto: QuickAddCustomerDto): Promise<string | null> => {
      try {
        const saved = await quickAddMutation(dto).unwrap();
        if (!saved?.cusId) {
          toast.error("The customer was saved but the server returned no id — pick them from the customer box.");
          return null;
        }
        toast.success(`${saved.cusName ?? dto.cusName} added.`);
        return saved.cusId;
      } catch (error) {
        // A 409 names the clashing ledger; it is shown as the server says it.
        toast.error(errorMessageOf(error));
        return null;
      }
    },
  ...
  // Re-tender (§22)
  // -------------------------------------------------------------------------
  const [fetchTenderContext] = useLazyGetTenderContextQuery();
  const [retenderMutation] = useRetenderBillMutation();
  const [retender, setRetender] = useState<SaleBillDraftApi["retender"]>(null);
  const openRetender = useCallback(
    async (key: BillKey, refno: string | null) => {
      const current = draftRef.current;
      // `rights.retender` is its own flag (§22 step 1), read off the loaded
      // bill when the pick IS the bill on screen; `/tender-context` does not
      // check rights, the post does (403).
      if (current.docId === key.sbId && current.rights && !current.rights.retender) {
        toast.warn("You don't have the re-tender right on this menu.");
        return;
      }
      setRetender({ key, refno, context: null, loading: true });
      try {
        const context = await fetchTenderContext(key).unwrap();
        setRetender({ key, refno: context.sbBillRefno ?? refno, context, loading: false });
      } catch (error) {
        setRetender(null);
        toast.error(errorMessageOf(error));
      }
  ...
  const closeRetender = useCallback(() => setRetender(null), []);
  const submitRetender = useCallback(
    async (voids: VoidPick[], replacements: BillTenderRow[], remark: string): Promise<boolean> => {
      const target = retender;
      if (!target?.context) {
        return false;
      }
      const current = draftRef.current;
      const onScreen = current.docId === target.key.sbId;
      // The builder wants the bill's OWN scope and date; when the picked bill
      // is not the one on screen, a scope-only stand-in carries the key.
      const scopeDraft: SaleBillDraft = onScreen
        ? current
        : {
            ...current,
            companyId: target.key.sbCompanyId,
            branchId: target.key.sbBranchId,
            accYear: target.key.sbAccYear,
            header: { ...current.header, billDate: target.context.sbBillDate.slice(0, 10) },
            customer: { ...current.customer, custId: null },
          };
      setBusy("saving");
      try {
        await retenderMutation(
          buildRetenderBody(target.key, target.context, voids, replacements, remark, scopeDraft, actor),
        ).unwrap();
        toast.success(`Bill ${target.refno ?? ""} re-tendered.`.replace("  ", " "));
        setRetender(null);
        // It reloads the bill only if the re-tendered bill is the one showing.
        if (onScreen) {
          await loadDocumentRef.current(target.key);
        }
        return true;
      } catch (error) {
        const notes = notesFromError(error);
        if (notes.length > 0 && onScreen) {
          dispatch(notesSet({ notes }));
          setNotesPopupOpen(true);
        } else {
          toast.error(errorMessageOf(error));
    ...
    [actor, dispatch, retender, retenderMutation],
  );
  ...
  const acceptRecovery = useCallback(
    (record: BillAutosave) => {
      const restored = draftFromAutosave(record);
      dispatch(draftReplaced(restored));
      void clearAutosave(actor.deviceId ?? "");
      void reconcileRestored(restored);
    },
    [actor.deviceId, dispatch, reconcileRestored],
  );
