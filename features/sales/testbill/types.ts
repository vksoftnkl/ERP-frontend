...
/** How a charge carried from an order is apportioned onto this bill. */
export type ChargeCarryBasis = "PRORATA" | "FULL" | "MANUAL" | "NONE";

/**
 * The carry facts stamped on a charge row at order import (§12.4), BEFORE its
 * id is cleared: which order charge it came from, in which year, and how the
 * bill's share of it is decided. `/validate` answers `proposals.charges[]`
 * keyed on the same pair, and the proposal is applied to rows whose basis is
 * neither MANUAL nor NONE.
 */
export type ChargeCarry = {
  /** The ORDER's `cdId`. */
  srcCdId: string;
  /** The order's own accounting year. */
  srcAccYear: string;
  basis: ChargeCarryBasis;
  /** The charge on the order, as raised. */
  orderAmount: number;
  /** Already billed on earlier bills against the order (from the proposal). */
  carriedSoFar: number;
  /** What the server proposes for this bill (from the proposal). */
  proposed: number;
  /** The server says this bill takes the order's last open quantity. */
  isFinalBill: boolean;
};

/**
 * One charge row on the bill: the shared draft row plus the carry facts. A
 * hand-added row has `carry` unset and sends no carry keys at all.
 */
export type BillChargeRow = DraftChargeRow & { carry?: ChargeCarry | null };
...
export type SaveBillChargeDto = {
  ...
  cdIsActive?: boolean;
  /** Scope (§12.4): null when empty; `cdVoucherNo` is always null on a bill. */
  cdCompId?: string | null;
  cdBranchId?: string | null;
  cdAccYear?: string | null;
  cdVoucherNo?: string | null;
  /** The charge's tax id — the rate is resolved from it, not from `ledGstRate` (§27 CHG-TAX). */
  cdTaxCode?: string | null;
  /** Carry keys — ONLY on a carried row (§12.4). */
  cdSrcCdId?: string;
  cdSrcAccYear?: string;
  cdCarryBasis?: ChargeCarryBasis;
