...
import { SettleDialog, type SettleContext } from "@/features/sales/testbill/components/settle-dialog";
...
import { lastFilledLineBefore } from "@/features/sales/testbill/state/factories";
import { isBillHold } from "@/features/sales/testbill/state/hold";
import type { BillAutosave } from "@/features/sales/testbill/state/hold";
import { totalAdjusted } from "@/features/sales/testbill/domain/validate";
import type {
  BillAdjustmentRow,
  SaleBillDocKey,
  SaleBillDraftLine,
...
import { useSaleBillDraft, type SaveOutcome } from "@/features/sales/testbill/state/use-bill-draft";
import { AdjustPanel } from "@/features/sales/testbill/components/adjust-panel";
import { AskText } from "@/features/sales/testbill/components/ask-text";
import { BillQuickStrip } from "@/features/sales/testbill/components/quick-strip";
import { CarryStrip } from "@/features/sales/testbill/components/carry-strip";
import { CustomerQuickAdd } from "@/features/sales/testbill/components/customer-quick-add";
import { IdentityStrip } from "@/features/sales/testbill/components/identity-strip";
import { OpenSourcesDialog } from "@/features/sales/testbill/components/open-sources-dialog";
import { ShippingDialog } from "@/features/sales/testbill/components/shipping-dialog";
import { RetenderDialog } from "@/features/sales/testbill/retender/retender-dialog";
import { RetenderPicker } from "@/features/sales/testbill/retender/retender-picker";
import { shippingModeOf, shippingSummary } from "@/features/sales/testbill/domain/shipping";
import { getUserInfo } from "@/lib/auth/session";
import { useBillVisibleSettings } from "@/features/sales/testbill/components/visible-settings";
import { BillListModal } from "@/features/sales/testbill/components/bill-list-modal";
import {
  BillCreditPanel,
  BillCustomerBlock,
  BillFactsLine,
  BillInfoBlock,
  BillPeopleBlock,
} from "@/features/sales/testbill/components/header-blocks";
import { SaleBillToolbar } from "@/features/sales/testbill/components/verb-bar";
import { AmendRemarkPrompt, CancelBillPrompt, CancelLinePrompt } from "@/features/sales/testbill/components/cancel-prompts";
import { ValidationPopup, WarningStrip } from "@/features/sales/testbill/components/warning-strip";
import { verbState } from "@/features/sales/testbill/state/verbs";
...
export type SaleBillScreenProps = {
...
export function SaleBillScreen({
  initialDocument,
  initialMode = "browse",
  onClose,
}: SaleBillScreenProps) {
  ...
  /** The shipping popup (§20). */
  const [shippingOpen, setShippingOpen] = useState(false);
  /** Quick add customer (§7.7). */
  const [quickAddOpen, setQuickAddOpen] = useState(false);
  /** Ctrl+F6 — the re-tender picker (§22). */
  const [retenderPickerOpen, setRetenderPickerOpen] = useState(false);
  /** Part-B (§21): the vehicle prompt, then the remark prompt. */
  const [partB, setPartB] = useState<{ step: "vehicle" | "remark"; vehicleNo: string } | null>(null);
  /** F11 — reprint the last bill this screen wrote (§26). */
   ...
   */
  const hasLines = draft.lines.some((line) => Boolean(line.itemId));
  const shippingSummaryText = useMemo(
    () => shippingSummary(draft.transport, { vehicleNo: draft.header.people.vehicleNo, driverName: draft.header.people.driverName }),
    [draft.header.people.driverName, draft.header.people.vehicleNo, draft.transport],
  );
  const shippingMode = useMemo(
    () => shippingModeOf({ status: draft.status, editable, locks: draft.locks, posting: draft.posting }),
    [draft.locks, draft.posting, draft.status, editable],
  );
  const openShipping = useCallback(() => {
    void api.loadBranchAddress();
    setShippingOpen(true);
  }, [api]);
  // §16.5 / §21: a validate note (or an e-way attempt on an empty band) marks
  // the dialog required and opens it.
  const seenTransportTick = useRef(api.transportRequiredTick);
  useEffect(() => {
    if (api.transportRequiredTick !== seenTransportTick.current) {
      seenTransportTick.current = api.transportRequiredTick;
      openShipping();
    }
  }, [api.transportRequiredTick, openShipping]);
  /** e-Way: Part-B once generated (§21), else generate — an empty band opens the popup. */
  const onEwaybill = useCallback(() => {
    if (api.ewaybill.isPartB) {
      setPartB({ step: "vehicle", vehicleNo: draft.header.people.vehicleNo });
      return;
    }
    void api.generateEwaybill();
  }, [api, draft.header.people.vehicleNo]);
  /** Ctrl+F6 (§22): the picker; a bill on screen with the right offers itself first. */
  const openRetenderPicker = useCallback(() => {
    setRetenderPickerOpen(true);
  }, []);
  /** F11 (§26): the last bill this screen wrote, as a reprint. */
  const reprintLastBill = useCallback(() => {
    if (!api.lastBillRef) {
      toast.info("Nothing printed yet on this screen — save or post a bill first.");
      return;
    }
    ...
    setReprintOpen(true);
  }, [api.lastBillRef, menuPermissions.canPrint]);
  /** Shift+F2 / Shift+F3: the e-invoice / e-way registers (menus 152 / 153). */
  const openGstRegister = useCallback((which: "einvoice" | "ewaybill") => {
    toast.info(
      which === "einvoice"
        ? "The e-invoice register (menu 152) is not built in this client yet."
        : "The e-way bill register (menu 153) is not built in this client yet.",
    );
  }, []);
  const registeredDeviceId = getUserInfo()?.deviceId ?? "";
  /** The Terms group lives inside the shipping popup (§20.1); the footer keeps Remarks. */
  const footerTermsFields = useMemo(
    () => ({
      ...visibleFields.terms,
      isVisible: (key: Parameters<typeof visibleFields.terms.isVisible>[0]) =>
  ...
  const shortcuts = {
    guardedRun,
    requestSave,
    appendChallan: () => void api.requestOpenSources("DC"),
    appendOrder: () => (hasLines ? void api.requestOpenSources("ORDER") : guardedRun("importOrder")),
    openRetenderPicker,
    reprintLastBill,
    openGstRegister,
    openTender,
      ...
      switch (event.key) {
        ...
        case "F5":
          ...
          if (event.ctrlKey || event.metaKey) {
            // Ctrl+F5: challan lines (§13.3).
            current.appendChallan();
            break;
          }
          current.requestSave();
          break;
        case "F4":
          if (event.ctrlKey || event.metaKey) {
            ...
            // below would otherwise swallow it. A bill with lines APPENDS
            // from the party's open orders instead (§13.3).
            event.preventDefault();
            current.appendOrder();
            break;
          }
        ...
        case "F6":
          ...
          // amend, the tender on its route (§6.3). Ctrl+F6 is re-tender (§22).
          event.preventDefault();
          if (event.ctrlKey || event.metaKey) {
            current.openRetenderPicker();
            break;
          }
        ...
        case "F3":
          ...
          if (event.shiftKey) {
            event.preventDefault();
            current.openGstRegister("ewaybill");
            break;
          }
        ...
        case "F2":
          event.preventDefault();
          if (event.shiftKey) {
            current.openGstRegister("einvoice");
            break;
          }
          current.onEdit();
          break;
        case "F11":
          // The last bill's reprint (§26). Never the browser's full-screen.
          event.preventDefault();
          current.reprintLastBill();
          break;
        default:
  ...
  return (
    <div className={quotationStyles.page}>
      <IdentityStrip draft={draft} onBack={() => guardedRun("back")} />
      ...
      <BillQuickStrip
        ...
        isNew={draft.isNewEntry}
        hasLines={hasLines}
        onQuickAddCustomer={() => setQuickAddOpen(true)}
        onImportQuotation={() => guardedRun("importQuotation")}
        // Ctrl+F4: a whole-document import on an EMPTY bill; an append from
        // the party's open orders once the bill has lines (§13.2, §13.3).
        onImportOrder={() => (hasLines ? void api.requestOpenSources("ORDER") : guardedRun("importOrder"))}
        onImportChallan={() => void api.requestOpenSources("DC")}
        onDiscountAll={() => setDiscountPromptOpen(true)}
        onAdjustPrices={() => setPricePromptOpen(true)}
        onShowCost={showLineCost}
        onShipping={openShipping}
        onEinvoice={() => void api.generateEinvoice()}
        onEwaybill={onEwaybill}
        onCopyAsNew={api.copyAsNew}
        einvoiceLabel={api.einvoice.label}
        ewaybillLabel={api.ewaybill.label}
        einvoiceEnabled={api.einvoice.enabled}
        ewaybillEnabled={api.ewaybill.enabled}
        einvoiceTooltip={api.einvoice.tooltip}
        shippingSummary={shippingSummaryText}
      />
      {/*
      ...
      <div className={quotationStyles.gridsRow}>
        ...
        <div className={quotationStyles.bottomRow}>
          ...
          >
            ...
            />
            <CarryStrip rows={draft.charges} editable={editable} onSetBasis={api.setCarryBasis} />
          </section>

          <TermsBlock
            terms={draft.terms}
            fields={footerTermsFields}
            disabled={!editable}
      ...
      */}
      ...
      />
      {/* §13.4 — the Open Sources dialog: appends, never replaces. */}
      <OpenSourcesDialog
        isOpen={api.openSources !== null}
        kind={api.openSources?.kind ?? "ORDER"}
        docs={api.openSources?.docs ?? []}
        loading={api.openSources?.loading ?? false}
        error={api.openSources?.error ?? null}
        onClose={api.closeOpenSources}
        onApply={(picks) => void api.applyOpenSources(picks)}
      />
      {/* §20 — the shipping popup: with the document on a draft, its own verb once posted. */}
      <ShippingDialog
        isOpen={shippingOpen}
        mode={shippingMode}
        required={draft.transportRequired}
        ewbVehicle={draft.posting?.ewb.vehicleNo ?? null}
        band={draft.transport}
        terms={draft.terms}
        vehicleNo={draft.header.people.vehicleNo}
        vehicleId={draft.header.people.vehicleId}
        companyId={draft.companyId}
        branchId={draft.branchId}
        shipTo={draft.party?.shipTo ?? []}
        billTo={{
          name: draft.customer.name,
          addr: draft.customer.address,
          place: draft.customer.place,
          pin: draft.header.custPin,
          phone: draft.customer.phone,
          stcd: draft.customer.stateCode,
          gstin: draft.customer.gstin,
        }}
        branch={api.branchAddress}
        busy={busy === "saving"}
        onClose={() => setShippingOpen(false)}
        onApply={api.applyShipping}
        onSaveTransport={api.saveTransport}
      />
      {/* §7.7 — quick add: the new customer is handled exactly like an operator pick. */}
      <CustomerQuickAdd
        isOpen={quickAddOpen}
        companyId={draft.companyId}
        branchId={draft.branchId}
        ...
        defaultPriceLevel={draft.header.priceLevel}
        busy={api.quickAddBusy}
        onClose={() => setQuickAddOpen(false)}
        onSubmit={async (dto) => {
          const id = await api.quickAddCustomer(dto);
          if (id) {
            setQuickAddOpen(false);
            onRequestCustomer(id);
          }
        }}
      />
      {/* §22 — re-tender: the picker (grid 115), then the dialog. */}
      <RetenderPicker
        isOpen={retenderPickerOpen}
        companyId={draft.companyId}
        branchId={draft.branchId}
        accYear={draft.accYear}
        deviceId={registeredDeviceId}
        onClose={() => setRetenderPickerOpen(false)}
        onPick={(key, refno) => {
          setRetenderPickerOpen(false);
          void api.openRetender(key, refno);
        }}
      />
      <RetenderDialog
        isOpen={api.retender !== null}
        context={api.retender?.context ?? null}
        loading={api.retender?.loading ?? false}
        masters={tenderMasters}
        billDate={api.retender?.context?.sbBillDate?.slice(0, 10) ?? draft.header.billDate}
        busy={busy === "saving"}
        onClose={api.closeRetender}
        onSubmit={(voids, replacements, remark) => void api.submitRetender(voids, replacements, remark)}
      />
      {/* §21 — Part-B: the vehicle, then the remark. A portal operation, never an edit of the band. */}
      <AskText
        isOpen={partB?.step === "vehicle"}
        title="Part-B — vehicle update"
        message={`The new vehicle on e-way bill ${draft.posting?.ewb.number ?? ""}.`}
        label="Vehicle number"
        required
        uppercase
        maxLength={20}
        initialValue={partB?.vehicleNo ?? ""}
        confirmLabel="Next"
        onCancel={() => setPartB(null)}
        onConfirm={(value) => setPartB({ step: "remark", vehicleNo: value })}
      />
      <AskText
        isOpen={partB?.step === "remark"}
        title="Part-B — reason"
        message="Optional. Why the vehicle changed."
        label="Remark"
        presets={["Breakdown", "Transhipment"]}
        maxLength={250}
        confirmLabel="Update e-way bill"
        onCancel={() => setPartB(null)}
        onConfirm={async (value) => {
          const pending = partB;
          setPartB(null);
          if (pending) {
            await api.updateEwaybillVehicle(pending.vehicleNo, value || null);
          }
        }}
      />
      {/* §26 — F11: the last bill this screen wrote, as a reprint (duplicate watermark). */}
      {reprintOpen && api.lastBillRef ? (
        <PrintOptionsDialog
          open
          onClose={() => setReprintOpen(false)}
          purposeCode={PURPOSE_CODE.SALE_INVOICE}
          documentLabel={api.lastBillRef.billRefno ? `Bill ${api.lastBillRef.billRefno} (reprint)` : "Bill (reprint)"}
          targets={[
            {
              docId: api.lastBillRef.sbId,
              companyId: api.lastBillRef.sbCompanyId,
              accYear: api.lastBillRef.sbAccYear,
              isReprint: true,
              filename: `bill-${api.lastBillRef.billRefno || api.lastBillRef.sbId}`,
            },
          ]}
        />
      ) : null}
